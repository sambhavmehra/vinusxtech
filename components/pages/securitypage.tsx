...
import { Shield, Lock, Eye, AlertTriangle, CheckCircle2, Terminal, ArrowUpRight, ShieldCheck, Radar, Bug, Fingerprint, ScanEye } from 'lucide-react';
              ...
              {vaptProcess.map((item, index) => (
                <Reveal key={item.step} delay={index * 0.1}>
                  ...
                  >
                    ...
                    {index < 3 && (
                      <div className="hidden lg:block absolute top-1/2 -right-3 w-6 h-[1px] bg-gradient-to-r from-[#00ff88]/30 to-transparent" />
                    )}
                  </motion.div>
                </Reveal>
              ))}
              ...
              {socProcess.map((item, index) => (
                <Reveal key={item.step} delay={index * 0.1}>
                  ...
                  >
                    ...
                    {index < 3 && (
                      <div className="hidden lg:block absolute top-1/2 -right-3 w-6 h-[1px] bg-gradient-to-r from-[#00d4ff]/30 to-transparent" />
                    )}
                  </motion.div>
                </Reveal>
              ))}
              ...
              {tools.map((tool, index) => (
                <Reveal key={tool.name} delay={index * 0.05}>
                  <motion.div
                    ...
                    className="glass-card rounded-2xl p-6 relative overflow-hidden group border border-white/5 cursor-pointer"
                  >
                    <div
                      className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
                      style={{ background: `linear-gradient(135deg, ${tool.color}05, transparent)` }}
                    />
                    ...
                    <div className="flex items-center gap-4 mb-3 relative z-10">
                      <div
                        ...
                        style={{ background: `${tool.color}10`, borderColor: `${tool.color}25` }}
                      >
          ...
          <div className="container mx-auto max-w-7xl relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {threats.map((threat, index) => (
                <Reveal key={threat.name} delay={index * 0.05}>
                  <motion.div
                    whileHover={{ x: 4 }}
                    className="flex items-center gap-4 p-4 rounded-2xl group cursor-default border border-white/5 transition-colors hover:bg-white/[0.02]"
                    style={{ background: 'rgba(255,255,255,0.01)' }}
                  >
                    <div
                      className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 border transition-transform duration-500 group-hover:scale-110"
                      style={{ background: `${threat.color}10`, borderColor: `${threat.color}25` }}
                    >
