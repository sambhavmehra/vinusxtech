...
import { Shield, Lock, KeyRound, LogOut, Check, X, Trash2, Star, Clock, Mail, MessageSquare, LayoutDashboard, Terminal, Activity, Menu, Settings, Bot } from 'lucide-react';
...
export default function AdminPage() {
    ...
    const handleReviewAction = async (id, action) => {
        if (action === 'delete' && !window.confirm('Purge this record?'))
            return;
        try {
    ...
    const handleDeleteMessage = async (id) => {
        if (!window.confirm('Purge this message permanently?'))
            return;
        try {
    ...
    if (status === 'loading') {
        return (<div className="min-h-screen bg-[#050508] flex items-center justify-center font-mono">
    ...
    if (status === 'unauth' || status === 'otp') {
        return (<div className="min-h-screen bg-[#050508] flex items-center justify-center p-4 relative overflow-hidden font-mono text-[#00ff88]">
        ...
        <motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.5, ease: "easeOut" }} className="w-full max-w-lg bg-black/80 border border-[#00ff88]/20 p-8 rounded-xl backdrop-blur-md shrink-0 z-10 shadow-[0_0_50px_rgba(0,255,136,0.1)] relative overflow-hidden">
          {/* Decorative scanline */}
          ...
          {error && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-6 p-4 bg-red-500/10 border border-red-500/50 text-red-500 rounded text-sm text-center uppercase tracking-wider font-bold">
              [SYSTEM WARNING] {error}
            </motion.div>)}

          <AnimatePresence mode="wait">
            {status === 'unauth' ? (<motion.form key="login" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} onSubmit={requestOtp}>
                <div className="mb-6">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" className="w-full bg-black/50 border border-[#00ff88]/30 rounded px-10 py-4 text-[#00ff88] placeholder-[#00ff88]/20 outline-none focus:border-[#00ff88] focus:bg-black transition-all font-mono" placeholder="identity..." value={username} onChange={(e) => setUsername(e.target.value.toLowerCase())} required/>
                  </div>
                </div>
                <button type="submit" disabled={loading || !username} className="w-full bg-[#00ff88]/10 border border-[#00ff88]/50 hover:bg-[#00ff88] hover:text-black hover:shadow-[0_0_30px_rgba(0,255,136,0.4)] text-[#00ff88] font-bold tracking-widest uppercase py-4 rounded flex items-center justify-center transition-all disabled:opacity-50 disabled:cursor-not-allowed group">
              ...
              </motion.form>) : (<motion.form key="otp" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} onSubmit={verifyOtp}>
                <div className="mb-8">
                  ...
                  <div className="relative flex justify-center">
                    <input type="text" className="w-[80%] bg-black/50 border border-[#00ff88]/30 rounded px-4 py-4 text-[#00ff88] placeholder-[#00ff88]/20 outline-none focus:border-[#00ff88] focus:ring-1 focus:ring-[#00ff88] transition-all text-center tracking-[0.75em] font-mono text-2xl uppercase" placeholder="XXXXXX" maxLength={6} value={otp} onChange={(e) => setOtp(e.target.value)} required/>
                  </div>
                </div>
                <button type="submit" disabled={loading || otp.length < 6} className="w-full bg-[#00ff88]/10 border border-[#00ff88]/50 hover:bg-[#00ff88] hover:text-black hover:shadow-[0_0_30px_rgba(0,255,136,0.4)] text-[#00ff88] font-bold tracking-widest uppercase py-4 rounded flex items-center justify-center transition-all disabled:opacity-50 disabled:cursor-not-allowed group">
                ...
                <button type="button" onClick={() => setStatus('unauth')} className="w-full mt-6 text-[#00ff88]/40 hover:text-[#00ff88] text-xs font-bold tracking-widest uppercase transition-colors">
                  [ Abort Process ]
                </button>
              </motion.form>)}
          </AnimatePresence>
    ...
    return (<div className="min-h-screen bg-[#050508] text-white flex flex-col md:flex-row selection:bg-[#00ff88]/30 selection:text-white">
      ...
      {/* Sidebar Navigation */}
      <AnimatePresence>
        <motion.aside initial={{ x: -100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className={`
          ...
          <div className="flex-1 px-4 py-2 space-y-2 overflow-y-auto">
            <button onClick={() => { setActiveTab('dashboard'); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'dashboard'
            ...
            <button onClick={() => { setActiveTab('reviews'); setIsSidebarOpen(false); }} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all ${activeTab === 'reviews'
            ...
            : 'text-gray-400 hover:bg-white/[0.02] hover:text-white border-l-2 border-transparent font-medium'}`}>
              ...
              {pendingReviews.length > 0 && (<span className="bg-purple-500/20 text-purple-400 text-[10px] px-2 py-0.5 rounded-full font-bold">{pendingReviews.length}</span>)}
            </button>
            <button onClick={() => { setActiveTab('messages'); setIsSidebarOpen(false); }} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all ${activeTab === 'messages'
            ...
            : 'text-gray-400 hover:bg-white/[0.02] hover:text-white border-l-2 border-transparent font-medium'}`}>
              ...
              {messages.length > 0 && (<span className="bg-[#00d4ff]/20 text-[#00d4ff] text-[10px] px-2 py-0.5 rounded-full font-bold">{messages.length}</span>)}
            </button>
            <button onClick={() => { setActiveTab('bot'); setIsSidebarOpen(false); }} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all ${activeTab === 'bot'
          ...
          <div className="p-6 border-t border-white/[0.05]">
            ...
            <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-red-500/10 text-red-500 hover:bg-red-500/20 transition-all font-bold text-sm tracking-wide">
      ...
      <main className="flex-1 max-h-screen overflow-y-auto bg-gradient-to-br from-[#0a0a0f] to-[#050508] relative">
        ...
        <div className="p-6 md:p-12 relative z-10 max-w-7xl mx-auto">
          {fetchingData && !isSidebarOpen ? (<div className="flex flex-col items-center justify-center h-[60vh]">
              ...
              {/* Dashboard Content */}
              {activeTab === 'dashboard' && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                  ...
                  <div className="mt-8">
                     ...
                     <div className="bg-[#0a0a0f] border border-white/[0.05] rounded-2xl p-6 shadow-xl">
                        {messages.length === 0 && reviews.length === 0 ? (<p className="text-gray-500 italic">No recent activity detected.</p>) : (<div className="space-y-4">
                              {messages.slice(0, 3).map(m => (<div key={m.id} className="flex items-center gap-4 border-b border-white/[0.05] pb-4 last:border-0 last:pb-0">
                              ...
                              {pendingReviews.slice(0, 2).map(r => (<div key={r.id} className="flex items-center gap-4 border-b border-white/[0.05] pb-4 last:border-0 last:pb-0">
              ...
              {/* Reviews Content */}
              {activeTab === 'reviews' && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                  <div className="flex items-end justify-between flex-wrap gap-4">
                    <div>
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      ...
                      {pendingReviews.length === 0 ? (<div className="bg-[#0a0a0f] border border-white/[0.05] border-dashed rounded-2xl p-10 flex flex-col items-center justify-center text-gray-500">
                          <Check className="w-10 h-10 text-gray-600 mb-2 opacity-50"/>
                          <p>All caught up!</p>
                        </div>) : (pendingReviews.map(review => (<ReviewCard key={review.id} review={review} onAction={handleReviewAction}/>)))}
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      {approvedReviews.length === 0 ? (<div className="bg-[#0a0a0f] border border-white/[0.05] border-dashed rounded-2xl p-10 flex flex-col items-center justify-center text-gray-500">
                          <p>No published reviews yet.</p>
                        </div>) : (approvedReviews.map(review => (<ReviewCard key={review.id} review={review} onAction={handleReviewAction}/>)))}
                    </div>
                  </div>
                </motion.div>)}

              {/* Messages Content */}
              {activeTab === 'messages' && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                  <div className="flex items-end justify-between flex-wrap gap-4">
                    <div>
                  ...
                  {messages.length === 0 ? (<div className="bg-[#0a0a0f] border border-white/[0.05] border-dashed rounded-3xl p-20 flex flex-col items-center justify-center text-gray-500 col-span-full shadow-lg">
                      <Mail className="w-16 h-16 text-gray-600 mb-4 opacity-30"/>
                      <p className="text-lg">Inbox is empty.</p>
                    </div>) : (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      {messages.map((msg) => (<motion.div key={msg.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-[#0a0a0f] border border-white/[0.05] hover:border-[#00d4ff]/30 rounded-2xl overflow-hidden transition-all duration-300 shadow-lg group flex flex-col">
                          <div className="p-6 border-b border-white/[0.05] bg-gradient-to-br from-white/[0.02] to-transparent">
                            ...
                            {(msg.company || msg.service) && (<div className="mt-4 flex flex-wrap gap-2">
                                {msg.company && (<span className="text-xs bg-white/[0.05] text-gray-300 px-3 py-1 rounded-full font-medium border border-white/10 uppercase tracking-widest">
                                    Org: {msg.company}
                                  </span>)}
                                {msg.service && (<span className="text-xs bg-[#00ff88]/10 text-[#00ff88] px-3 py-1 rounded-full font-medium border border-[#00ff88]/20 uppercase tracking-widest">
                                    Need: {msg.service}
                                  </span>)}
                              </div>)}
                          </div>
                          ...
                          <div className="p-4 border-t border-white/[0.05] bg-[#0a0a0f] flex justify-end">
                            <button onClick={() => handleDeleteMessage(msg.id)} className="text-gray-500 hover:text-red-500 flex items-center gap-2 text-sm font-bold uppercase tracking-wider transition-colors px-3 py-1.5 rounded-lg hover:bg-red-500/10">
              ...
              {/* Bot Config Content */}
              {activeTab === 'bot' && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                  <div className="flex items-end justify-between flex-wrap gap-4">
                    <div>
                  ...
                  <div className="bg-[#0a0a0f] border border-white/[0.05] rounded-3xl p-6 md:p-8 shadow-lg">
                    ...
                    <textarea value={systemPrompt} onChange={(e) => setSystemPrompt(e.target.value)} rows={10} className="w-full bg-black/50 border border-white/[0.1] focus:border-orange-500/50 rounded-xl p-4 text-gray-300 font-mono text-sm outline-none transition-all resize-y custom-scrollbar shadow-inner" placeholder="Enter the system prompt here..."/>
                    
                    <div className="mt-6 flex justify-end">
                      <button onClick={saveSystemPrompt} disabled={savingPrompt} className="bg-gradient-to-br from-orange-500 to-red-600 text-white font-bold px-8 py-3 rounded-xl hover:scale-[1.02] shadow-[0_0_20px_rgba(249,115,22,0.3)] transition-all disabled:opacity-50 flex items-center justify-center min-w-[150px]">
                        {savingPrompt ? (<Activity className="w-5 h-5 animate-pulse"/>) : (<>Save Logic <Check className="w-5 h-5 ml-2"/></>)}
                      </button>
                    </div>
                  </div>
                </motion.div>)}
...
function ArrowRightIcon(props) {
    return (<svg {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3"/>
    </svg>);
}
function ReviewCard({ review, onAction }) {
    return (<div className="bg-[#0a0a0f] border border-white/[0.05] rounded-2xl overflow-hidden transition-all hover:border-white/[0.1] shadow-lg flex flex-col">
      <div className="p-6 border-b border-white/[0.05]">
        <div className="flex justify-between items-start gap-4 mb-4">
          <div>
            ...
            {(review.role || review.company) && (<p className="text-xs text-purple-400 mt-2 font-bold uppercase tracking-widest bg-purple-500/10 inline-block px-2 py-1 rounded">
      ...
      <div className="flex bg-[#0a0a0f] p-2">
        {!review.approved ? (<button onClick={() => onAction(review.id, 'approve')} className="flex-1 bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500 hover:text-black py-3 rounded-lg flex items-center justify-center gap-2 text-sm font-bold uppercase tracking-widest transition-all">
            <Check className="w-4 h-4"/> Publish
          </button>) : (<button onClick={() => onAction(review.id, 'unapprove')} className="flex-1 bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500 hover:text-black py-3 rounded-lg flex items-center justify-center gap-2 text-sm font-bold uppercase tracking-widest transition-all">
            <X className="w-4 h-4"/> Revoke
          </button>)}
        <div className="w-[1px] bg-white/[0.05] mx-2 my-1"/>
        <button onClick={() => onAction(review.id, 'delete')} className="flex-none w-14 bg-red-500/5 text-red-500 hover:bg-red-500 hover:text-white rounded-lg transition-all flex items-center justify-center" title="Delete Record">
